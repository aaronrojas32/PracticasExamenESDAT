...
/**
 * Gestiona una cola de productos con múltiples copias,
 * devolviendo siempre el producto con menos copias restantes.
...
public class GestorProductos<E> {
    ...
    private final Queue<ProductoDuplicado<E>> cola;

    /**
     * Crea un gestor vacío.
     */
    public GestorProductos() {
        this.cola = new PriorityQueue<>(Comparator.comparingInt(ProductoDuplicado::getCopias));
    }
    ...
    public void addProducto(E producto, int copias) {
        ProductoDuplicado<E> pd = new ProductoDuplicado<>(producto, copias);
        cola.offer(pd);
    }

    /**
     * Obtiene el siguiente producto a procesar, decrementando sus copias.
     * Si tras la extracción quedan copias, se reintroduce en la cola.
     * @return El producto, o null si no hay más.
     */
    public E siguienteProducto() {
        ProductoDuplicado<E> pd = cola.poll();
        if (pd == null) {
            return null;
        }
        E resultado = pd.getProducto();
        pd.descontarCopia();
        if (pd.getCopias() > 0) {
            cola.offer(pd);
        }
